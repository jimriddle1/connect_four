...
RSpec.describe Turn do
  ...
  xit "checks diag_down_right win condition_4" do
    board1 = Board.new
    board1.board_hash[:A] = ['X','.','O','.','.','.']
  ...
  xit "checks diag_up_right win condition_1" do
    board1 = Board.new
    board1.board_hash[:A] = ['X','.','O','.','.','.']
    board1.board_hash[:B] = ['X','.','.','.','X','.']
    board1.board_hash[:C] = ['O','.','X','X','O','.']
    board1.board_hash[:D] = ['X','O','X','.','O','O']
    board1.board_hash[:E] = ['.','X','X','.','.','O']
    board1.board_hash[:F] = ['.','.','.','O','.','.']
    board1.board_hash[:G] = ['.','.','O','X','.','.']
    turn = Turn.new(board1)
    expect(turn.check_diagonal_up_right).to eq("X")
  end
  xit "checks diag_up_right win condition_2" do
    board1 = Board.new
    board1.board_hash[:A] = ['X','.','O','.','.','.']
    board1.board_hash[:B] = ['X','.','.','.','X','.']
    board1.board_hash[:C] = ['O','.','X','X','O','.']
    board1.board_hash[:D] = ['X','O','X','.','O','O']
    board1.board_hash[:E] = ['.','.','X','.','O','O']
    board1.board_hash[:F] = ['.','.','.','O','.','.']
    board1.board_hash[:G] = ['.','.','O','X','.','.']
    turn = Turn.new(board1)
    expect(turn.check_diagonal_up_right).to eq("O")
  end
